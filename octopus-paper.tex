...
\begin{abstract}\textbf{
    Haplotype-based variant callers, which consider physical linkage between variation sites, are recognized to give the best results for germline variation discovery and genotyping.
    However, there is a lack of haplotype-based callers that can handle experimental designs beyond detecting common germline variation in diploid individuals.
    Here we present Octopus, a flexible haplotype-based variant detection algorithm with a polymorphic Bayesian genotyping model capable of handling a range of experimental designs.
    Beside offering the first general approach to probabilistic phasing directly from raw sequence data, Octopus outperforms all existing germline, somatic, and \emph{de novo} mutation detection tools.
    Because of its modular structure, it can be easily extended to handle atypical samples and new sequencing technology.    
}\end{abstract}

\invisiblesection{Motivation}

Detection of genetic variation and genotyping from high-throughput sequencing (HTS) data is now commonplace in clinical diagnosis pipelines.
This has led to the development of a number of variant detection algorithms \cite{RN167}.
Other than some based on \emph{de novo} assembly \cite{RN148}, the majority of these algorithms require sequencing reads first to be mapped to a reference sequence using a read mapper \cite{RN141, RN538, RN5}.
Na\"\i ve variant callers make calls on a positional basis, using the read alignments provided from a read mapper \cite{RN537}.
By making the simplifying assumption that these read alignments are always true, positional callers are prone to errors, particularly around indel mutations.
In contrast, haplotype-based methods only assume that the read mapping location is approximately correct, and re-align reads to consider evidence for multiple hypothetical haplotypes.
This is an improvement for two reasons. First, alignment errors introduced by read mappers are avoided, reducing dependence on the read mapper.
Second, the genotype posterior signal-to-noise ratio improves with haplotype length because the number of erroneous haplotypes increases exponentially while the number of true haplotypes remains constant.
This makes haplotype based methods more powerful than equivalent positional methods \cite{RN166}.
